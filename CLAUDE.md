...
### Roles y acceso
- **El schema tiene `role UserRole @default(EDITOR)`.** Todo usuario nuevo que haga login via Google OAuth queda como `EDITOR` automáticamente. Si ves que un usuario con acceso legítimo ve la lista de clientes vacía, probablemente su rol es `EDITOR` y `ClientUser` no tiene entradas para ese email. Para promover a ADMIN: `UPDATE "User" SET role='ADMIN' WHERE email='...'` directamente en BD de producción (via Easypanel Console → Sh → node con PrismaClient). No hay UI de gestión de roles en v1.
- **`ClientUser` vacío = EDITOR ve lista vacía.** La tabla `ClientUser` asocia EDITORs con clientes por email. Si está vacía y el usuario es EDITOR, verá cero clientes. Resolver antes de dar acceso al equipo (Félix, Cindy).
